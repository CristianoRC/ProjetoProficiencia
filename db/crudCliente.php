...
function cadastrar()
{
    ...
    preg_match("/^\d{3}\.\d{3}\.\d{3}\-\d{2}$/", $cpf, $mathesCPF);
    if (count($mathesCPF) == 0) {
        $parametros .= "_cpfFormato=CPF com formato Invalido&";
    }

    preg_match("/^\([1-9]{2}\) (?:[2-8]|9[1-9])[0-9]{3}\-[0-9]{4}$/", $telefone, $mathesTelefone);
    if (count($mathesTelefone) == 0) {
        $parametros .= "_telefoneFormato=Telefone com formato Invalido&";
    }
    ...
    if ($parametros == "") {
        ...
        header("Location: http://localhost/criarCliente.php?$parametros", true, 301);
        exit();
    }
}
